...
for(let i = 0; i < dito.abilities.length; i++){console.log(dito.abilities[i].ability.name)}
dito.base_experience
dito.height
dito.held_items.filter(item => item.item.name ==="metal-powder" )
dito.sprites 
dito.stats.map(item => item.stat.name)
dito.types 
dito.weight
dito.version_group_details 
dito.moves[0].version_group_details[0]
